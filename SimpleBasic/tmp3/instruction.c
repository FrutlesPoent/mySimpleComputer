...
Variable* newVar(char var) {
    Variable* varStruct = malloc(sizeof(*varStruct));
    if (varStruct == NULL)
        return varStruct;
...
Inst* newInstruction(String* instruction, int location) {
    Inst* inst = malloc(sizeof(Instruction)); // create structure
    if (inst == NULL)
        return inst;
...
void addInstructionSecond(char* inst, int operand) {
    ...
    if (instruction->instructions == NULL) { // if empty instructions then give some memory
        instruction->instructions = malloc(sizeof(Inst) * 10);
        instruction->instructionListCapacity = 10; // increase capacity of the list
    }

    if (instruction->instructionListSize == instruction->instructionListCapacity) { // realoc if we reached max use of memory
...
void storeAccum(char var) {
    Variable* varStructure = findVarS(var, 1);
    addInstructionSecond("STORE", varStructure->location); // store in accum
}
...
int isVar(int var) {
    ...
    if (varStruct->varName < 'A') // is it number or symbol
        return 1;
    else
        return 0;
}
